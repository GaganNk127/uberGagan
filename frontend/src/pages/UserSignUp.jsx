...
import { useNavigate } from 'react-router-dom'
 ...
 function UserSignUp() {
    ...
    const submitHandler = async (e)=>{
        ...
        if (response.status === 201) {
         ...
         setUser(data.user);  // Capital U
         console.log('User saved:', data.user);
         localStorage.setItem('token',data.token)
            setTimeout(() => {
                 navigate('/home');
             }, 0); // navigate after state is set
}
 ...
 <input className='bg-[#eeeeee] w-1/2 px-2 py-3 text-lg  rounded '
     ...
     <button className='flex items-center justify-center bg-black w-full text-white py-3 rounded mt-4'>Sign-up</button>
     <Link to={'/login'} className='text-blue-600'><span className= 'text-black ml-8'>Alreadu Have an Account? </span>Login</Link>
 </form>
    </div>
   <p>
